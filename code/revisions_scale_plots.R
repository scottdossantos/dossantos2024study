# redo: comparing ALDEx2 with and without scale simulation: london/europe dataset
...
#       blue outlines = significant feature with absolute effect size >1
...
# ----------- NEW FIGURE ---------- more transparent, larger blue halo

# png(paste0(path.to.github,"figs_for_paper/R_FigsLondonEurope/suppl_lon_eur_scaleMWs_redo.png"),
#     units = "in", height = 6, width = 12, res = 600)
...
points(aldex.le.ns.clr.all$diff.win[abs(aldex.le.ns.clr.all$effect) > 1 & aldex.le.ns.clr.all$we.eBH <0.01],
       aldex.le.ns.clr.all$diff.btw[abs(aldex.le.ns.clr.all$effect) > 1 & aldex.le.ns.clr.all$we.eBH <0.01],
       pch = 21, col = rgb(0,0,1,0.35), cex = 0.9)
abline(h = 0, col = rgb(0,0,1,0.5), lty = 2)
