...
for(i in levels(factor(tax.vec.loneur.ls))){
  reads.loneur.total[[i]] <- colSums(filt.ls.loneur[which(tax.vec.loneur.ls == i),])
}

# add info for all taxa with no known taxonomy in VIRGO
reads.loneur.total[["Unknown"]]<-colSums(filt.ls.loneur[which(is.na(tax.vec.loneur.ls)),])
...
for(i in levels(factor(tax.vec.loneur.ls))){
  n.genes.loneur[i] <- length(which(tax.vec.loneur.ls == i))
}

# get total number of genes in species with no known taxonomy in VIRGO
n.genes.loneur["Unknown"]<-length(which(is.na(tax.vec.loneur.ls)))
...
length(which(rowSums(bars.loneur.df) == colSums(reads.loneur.df))) # all TRUE
...
bars.loneur.cols[19,1] <- "grey80"
bars.loneur.cols[20,1] <- "grey35"
rownames(bars.loneur.cols)[19] <- "Other"
rownames(bars.loneur.cols)[20] <- "Unknown"
...
# png("~/BVSubgroups_lon_eur_relAbund_redo_filtered.png",
...
for(i in levels(factor(tax.vec.virg.ls))){
  reads.virg.total[[i]] <- colSums(filt.ls.virg[which(tax.vec.virg.ls == i),])
}

# add info for all taxa with no known taxonomy in VIRGO
reads.virg.total[["Unknown"]]<-colSums(filt.ls.virg[which(is.na(tax.vec.virg.ls)),])
...
n.genes.virg <- vector()
for(i in levels(factor(tax.vec.virg.ls))){
  n.genes.virg[i] <- length(which(tax.vec.virg.ls == i))
}

# get total number of genes in species with no known taxonomy in VIRGO
n.genes.virg["Unknown"]<-length(which(is.na(tax.vec.virg.ls)))
...
bars.virg.prop.df <- bars.virg.prop.df %>% 
  relocate(sample, .before = Anaerococcus_lactolyticus)
...
bars.virg.plot$species <- case_when(bars.virg.plot$species == "BVAB1" ~ "BVAB1",
                                  ...
                                  bars.virg.plot$species == "Prevotella sp." ~ glue("<i>Prevotella</i> sp."),
...
rownames(bars.virg.cols)[c(1,4,6:8,10,13,19,20,25,28,32,33:35)] <- c("Anaerococcus_lactolyticus","Bifidobacterium_breve",
                                                                  "Corynebacterium_aurimucosum","Corynebacterium_pseudogenitalium",
                                                                  "Finegoldia_magna","Lactobacillus_coleohominis",
                                                                  "Lactobacillus_helveticus","Mycoplasma_hominis","Peptoniphilus_harei",
                                                                  "Prevotella_disiens","Propionimicrobium_lymphophilum",
                                                                  "Staphylococcus_epidermidis","Streptococcus_anginosus",
                                                                  "Other","Unknown")

bars.virg.cols[c(1,4,6:8,10,13,19,20,25,28,32,33:35),1] <- c("deeppink4","gold4","cyan3",
...
# plot microbiome composition from metatranscriptome reads of known taxonomy
# for taxa represented by >75 genes
# png("~/BVSubgroups_virginia_relAbund_filtered.png",
#     units = "in", height = 4, width = 8, res = 400)
...
bars.virg.plot %>% 
  ...
  xlab("Samples")+ylab("Relative abundance (%)")+ labs(fill="<b>Taxon</b>")+
  theme_bw()+
  theme(legend.text = element_markdown(colour = "black", size = 5),
        legend.title = element_markdown(colour = "black", size = 8),
        legend.key.size = unit(0.2,"cm"), legend.box.spacing = unit(0.05, "cm"),
